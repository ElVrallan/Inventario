...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800">{{ isset($proveedore) ? 'Editar' : 'Nuevo' }} Proveedor</h1>
    </div>

    <div class="bg-white shadow-sm rounded-lg p-6">
        <form action="{{ isset($proveedore) ? route('proveedores.update', ['proveedore' => $proveedore->id]) : route('proveedores.store') }}" method="POST">
            @csrf
            @if(isset($proveedore))
                @method('PUT')
            @endif
